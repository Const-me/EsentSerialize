...
namespace EsentSerialization
{
	...
	/// <remarks>You must ( Commit() or LazyCommit() or Rollback() ) and/or Dispose() the transaction, otherwise the unmanaged resources will leak enormously fast.</remarks>
	public interface iSerializerTransaction : IDisposable
	{
		...
		/// <summary>Commit the transaction, but this API does not wait for the transaction to be flushed to the transaction log file before returning to the caller. Doing so will close this transaction.</summary>
		void LazyCommit();

		/// <summary>Rollback the transaction. Doing so will close this transaction.</summary>
